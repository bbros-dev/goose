...
//!
//! Swanling can generate a number of log files during a load test, enabled through any combination of
...
//! [`SwanlingUser`](../swanling/struct.SwanlingUser.html)s sending log messages through the same shared
...
//!
//! Note: there's also a `--swanling-log` run time option which records any errors or messages
//! generated by Swanling while running a load test. This functionality is not implemented in this
//! file.
//!
//! ## Request File logger
//! The Swanling requests logger is enabled with the `--request-log` command-line option, or the
//! [`SwanlingDefault::RequestLog`](../enum.SwanlingDefault.html#variant.RequestLog) default
//! configuration option. The format of the log is configured with the `--request-format`
//! command-line option, or the
//! [`SwanlingDefault::RequestFormat`](../enum.SwanlingDefault.html#variant.RequestFormat) default
//! configuration option.
//!
//! Each [`SwanlingRequestMetric`] object generated by all [`SwanlingUser`](../swanling/struct.SwanlingUser.html)
//! threads during a load test is written to this log file.
//!
//! ## Task File logger
//! The Swanling tasks logger is enabled with the `--task-log` command-line option, or the
//! [`SwanlingDefault::TaskLog`](../enum.SwanlingDefault.html#variant.TaskLog) default
//! configuration option. The format of the log is configured with the `--task-format`
//! command-line option, or the
//! [`SwanlingDefault::TaskFormat`](../enum.SwanlingDefault.html#variant.TaskFormat) default
//! configuration option.
//!
//! Each [`SwanlingTaskMetric`] object generated by all [`SwanlingUser`](../swanling/struct.SwanlingUser.html)
//! threads during a load test is written to this log file.
//!
//! ## Debug File logger
//! The Swanling debug logger is enabled with the `--debug-log` command-line option, or the
//! [`SwanlingDefault::DebugLog`](../enum.SwanlingDefault.html#variant.DebugLog) default
//! configuration option.
//!
//! Each [`SwanlingDebug`] object generated by all [`SwanlingUser`](../swanling/struct.SwanlingUser.html)
//! threads during a load test is written to this log file.
//!
//! ### Writing Debug Logs
//! Logs can be sent to the logger thread by invoking
//! [`log_debug`](../swanling/struct.SwanlingUser.html#method.log_debug) from load test task functions.
//!
//! Calls to
//! [`set_failure`](../swanling/struct.SwanlingUser.html#method.set_failure)
//! automatically invoke
//! [`log_debug`](../swanling/struct.SwanlingUser.html#method.log_debug).
//!
...
//! [`log_debug`](../swanling/struct.SwanlingUser.html#method.log_debug) documentation.
//!
...
/// Optional unbounded sender from all SwanlingUsers to logger thread, if enabled.
...
/// Traits for SwanlingDebug logs.
impl SwanlingLogger<SwanlingDebug> for SwanlingConfiguration {
    /// Converts a SwanlingDebug structure to a formatted string.
    fn format_message(&self, message: SwanlingDebug) -> String {
        if let Some(debug_format) = self.debug_format.as_ref() {
            match debug_format {
                ...
                // Raw format is Debug output for SwanlingRawRequest structure.
    ...
    /// Converts a SwanlingDebug structure to a CSV row.
...
impl SwanlingLogger<SwanlingErrorMetric> for SwanlingConfiguration {
    ...
    /// Converts a SwanlingErrorMetric structure to a CSV row.
...
/// Traits for SwanlingRequestMetric logs.
impl SwanlingLogger<SwanlingRequestMetric> for SwanlingConfiguration {
    /// Converts a SwanlingRequestMetric structure to a formatted string.
    fn format_message(&self, message: SwanlingRequestMetric) -> String {
        if let Some(request_format) = self.request_format.as_ref() {
            match request_format {
                // Use serde_json to create JSON.
                SwanlingLogFormat::Json => json!(message).to_string(),
                // Raw format is Debug output for SwanlingRequestMetric structure.
                SwanlingLogFormat::Raw => format!("{:?}", message),
                // Not yet implemented.
                SwanlingLogFormat::Csv => self.prepare_csv(&message),
            }
    ...
    /// Converts a SwanlingRequestMetric structure to a CSV row.
...
impl SwanlingLogger<SwanlingTaskMetric> for SwanlingConfiguration {
    ...
    /// Converts a SwanlingTaskMetric structure to a CSV row.
...
impl SwanlingConfiguration {
    /// Makes sure the SwanlingConfiguration has any/all configured log files (loading from defaults
    ...
    pub(crate) async fn setup_loggers(
        ...
        // Create an unbounded channel allowing SwanlingUser threads to log errors.
    ...
    pub(crate) async fn logger_main(
        ...
        // Loop waiting for and writing error logs from SwanlingUser threads.
